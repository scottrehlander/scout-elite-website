...
### Prototypes
Keep It In (blue-line pinch timing), Shootout (goalie learns your shot tendencies across visits, persisted), Coach's Challenge (frame-scrub offside calls), The Telestrator (draw a route, players run it literally), Bar Down (one-touch tap-to-loft against a clock). Play events only, except Bar Down, which fires `arcade_complete` with `score`, `goals`, `streak`.

**Bar Down** is the Flappy Dunk formula in hockey dress (see the backlog entry it came from). Two things learned the hard way while tuning it, both worth keeping: the crossbar originally swatted away every puck that fell on the net, so the only way to score was a low horizontal snipe and a fast tapper scored nothing at all; the front 62% of the mouth (`OPEN_FRAC`) is now open from above so you can drop it in, which is the whole point of the genre. And the puck escaped off the top of the screen until a glass ceiling was added at `GLASS_Y`. Tune it by sweeping a metronome bot across tap cadences and checking the score spread, but do not tune *to* the bot: a metronome cannot play this game at fast cadences and a human can, because a human watches the puck.

### Games backlog
- ~~**Bar Down (one-touch tap-to-loft, timed):**~~ **Built 2026-08-13**, see the Prototypes section. the Flappy Dunk / Ziosk-tabletop-basketball formula in hockey dress, requested 2026-08-13 (Scott's kids play the basketball one at Texas Roadhouse). Tap to loft the puck upward, keep it airborne, and snipe it into a net; the run is on a countdown that scoring extends, so a hot streak plays forever and a cold one ends fast. Streak multipliers reward clean snipes, with the top tier being an actual bar-down (off the crossbar and in). The basketball original wraps the ball around the screen edges; hockey has a better answer, since the puck should ricochet off the boards, which is both more authentic and the same "it comes back at you" feel. Loud, physical, zero-instruction, and the only arcade game we would have with a real risk/reward score chase.
